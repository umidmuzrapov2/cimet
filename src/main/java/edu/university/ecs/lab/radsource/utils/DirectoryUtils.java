...
public class DirectoryUtils {
  public static String[] getMsPaths(String root) {
    ...
    //        } catch (Exception e) {
    //            // log this somewhere
    //            System.out.println("Failed to read folder exclusions, will parse all folders: " +
    // e.getMessage());
    //        }
    //
    ...
    ////            e.printStackTrace();
    ////        }
    //        return rootDir.list((current, name) -> new File(current, name).isDirectory() &&
    // !ignoreFolders.contains(name));
  }
  ...
  private static File[] filterDirectories(String root) {
    ...
    return rootDir.listFiles(
        (current, name) -> new File(current, name).isDirectory() && !ignoreFolders.contains(name));
  }
    ...
    if (ignoreFolders == null) {
      ...
      try {
        ...
        InputStream inputStream =
            DirectoryUtils.class.getClassLoader().getResourceAsStream("ignoreFolders.json");
        Reader ignoreReader = new InputStreamReader(inputStream);
        ignoreFolders.addAll(Arrays.asList(gson.fromJson(ignoreReader, String[].class)));
      } catch (Exception e) {
        // log this somewhere
        System.out.println(
            "Failed to read folder exclusions, will parse all folders: " + e.getMessage());
      }
    }
