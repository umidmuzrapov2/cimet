...
import edu.university.ecs.lab.radsource.rad.context.RestFlowContext;
...
/**
 * This class constructs a {@link RestFlowContext}. It takes a list of {@link RestEntityContext} as
 * input. It matches client entities with server entities based on url and other properties.
 *
 * @author Dipta Das
 */
@Service
public class RestFlowService {
  ...
  private List<RestFlow> getRestFlows(
      List<RestEntity> serverEntities, List<RestEntity> clientEntities) {
  ...
  private List<RestFlow> getPossibleRestFlows(
      List<RestEntity> serverEntities, List<RestEntity> clientEntities) {
    List<RestFlow> restFlows = new ArrayList<>();
