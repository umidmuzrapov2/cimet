package edu.university.ecs.lab.radsource.rad.dataflow;

import edu.university.ecs.lab.radsource.rad.instruction.IndexWrapper;
import edu.university.ecs.lab.radsource.rad.instruction.InstructionInfo;
import edu.university.ecs.lab.radsource.rad.instruction.StringStackElement;
import edu.university.ecs.lab.radsource.rad.model.HttpMethod;

import java.util.ArrayList;
import java.util.List;

/**
 * This class detects the argument of a method call from a list of {@link
 * edu.university.ecs.lab.radsource.rad.instruction.InstructionInfo}. It constructs a list of {@link
 * edu.university.ecs.lab.radsource.rad.instruction.StringStackElement}.
 *
 * @author Dipta Das
 */
public class LocalVariableScanner {

  public static int findIndexForMethodCall(List<InstructionInfo> instructions, String method)
  ...
  public static List<StringStackElement> peekParamForMethodCall(
      List<InstructionInfo> instructions, int index, int numberOfParams) throws DataFlowException {
    // skip through
    for (index = index - 1; index >= 0 && index < instructions.size(); index--) {
      InstructionInfo instruction = instructions.get(index);
      String opcode = instruction.getOpcode();

      if (opcode.contains("ldc")
          || opcode.contains("aload")
          || opcode.contains("iload")
          || opcode.contains("getstatic")) {
        numberOfParams--;
      }
  ...
  public static List<StringStackElement> peekImmediateStringVariable(
      List<InstructionInfo> instructions, int index) throws DataFlowException {
    ...
    for (index = index - 1; index >= 0 && index < instructions.size(); index--) {
      ...
      if (getLDC(instruction) != null) {
        ...
        StringStackElement stringStackElement =
            new StringStackElement(
                StringStackElement.StringStackElementType.CONSTANT, getLDC(instruction));
        ...
        if (fieldAccess) { // must have aload_0 before field access
          ...
          try {
            ...
            stringStackElements.addAll(
                peekImmediateStringVariable(instructions, storeIndex)); // recursive call
          } catch (
              DataFlowException e) { // not declared inside the method, possibly method parameter
            StringStackElement stringStackElement =
                new StringStackElement(
                    StringStackElement.StringStackElementType.PARAM, Integer.toString(pointer));
            stringStackElements.add(stringStackElement);
          }
        }
        ...
        StringStackElement stringStackElement =
            new StringStackElement(
                StringStackElement.StringStackElementType.FIELD, getFieldAccess(instruction));
        stringStackElements.add(stringStackElement);
  ...
  private static boolean isStringBuilderAppend(InstructionInfo instruction) {
    if (instruction.getOpcode().equals("invokevirtual")
        && instruction.getInstruction() instanceof IndexWrapper) {
      IndexWrapper indexWrapper = (IndexWrapper) instruction.getInstruction();
  ...
  private static boolean isStringBuilderInit(InstructionInfo instruction) {
    if (instruction.getOpcode().equals("invokespecial")
        && instruction.getInstruction() instanceof IndexWrapper) {
      IndexWrapper indexWrapper = (IndexWrapper) instruction.getInstruction();
  ...
  private static String getFieldAccess(InstructionInfo instruction) {
    if (instruction.getOpcode().equals("getfield")
        && instruction.getInstruction() instanceof IndexWrapper) {
      IndexWrapper indexWrapper = (IndexWrapper) instruction.getInstruction();
  ...
  public static HttpMethod peakHttpMethodForExchange(
      List<InstructionInfo> instructions, int index) {
    for (index = index - 1; index >= 0 && index < instructions.size(); index--) {
      InstructionInfo instruction = instructions.get(index);

      if (instruction.getOpcode().equals("getstatic")
          && instruction.getInstruction() instanceof IndexWrapper) {
        IndexWrapper indexWrapper = (IndexWrapper) instruction.getInstruction();
  ...
  public static int peekImmediateStoreIndex(
      List<InstructionInfo> instructions, int index, int pointer) throws DataFlowException {
    for (index = index - 1; index >= 0 && index < instructions.size(); index--) {
      InstructionInfo instruction = instructions.get(index);
  ...
  public static String getLDC(InstructionInfo instruction) {
    if (instruction.getOpcode().equals("ldc")
        && instruction.getInstruction() instanceof IndexWrapper) {
      IndexWrapper indexWrapper = (IndexWrapper) instruction.getInstruction();
