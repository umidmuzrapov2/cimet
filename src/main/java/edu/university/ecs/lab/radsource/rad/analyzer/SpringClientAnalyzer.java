package edu.university.ecs.lab.radsource.rad.analyzer;

import edu.university.ecs.lab.radsource.rad.dataflow.DataFlowException;
import edu.university.ecs.lab.radsource.rad.dataflow.LocalVariableScanner;
import edu.university.ecs.lab.radsource.rad.instruction.InstructionInfo;
import edu.university.ecs.lab.radsource.rad.instruction.InstructionScanner;
...
/**
 * This class detects Spring Boot RestTemplate client calls. It can detect both url and return type
 * of a rest call. It uses {@link
 * edu.university.ecs.lab.radsource.rad.dataflow.LocalVariableScanner} to detect the url. It
 * constructs a list of {@link edu.university.ecs.lab.radsource.rad.model.RestEntity} of rest
 * clients.
 *
 * @author Dipta Das
 */
  ...
  public List<RestEntity> getRestEntity(CtClass ctClass, Properties properties) {
    ...
    for (CtMethod ctMethod : ctClass.getMethods()) {
      ...
      for (RestTemplateMethod targetMethod : restTemplateMethods) {
        try {
          index =
              LocalVariableScanner.findIndexForMethodCall(
                  instructions, restTemplateClass + "." + targetMethod.restTemplateMethod);
      ...
      if (foundMethod.restTemplateMethod.equals("exchange")) {
        foundMethod.httpMethod =
            LocalVariableScanner.peakHttpMethodForExchange(instructions, index);
        log.info(ctMethod.getLongName() + " exchange type " + foundMethod.httpMethod.toString());
      }

      // find url
      try {
        List<StringStackElement> stringStackElements =
            LocalVariableScanner.peekParamForMethodCall(
        ...
        for (StringStackElement stringStackElement : stringStackElements) {
          if (stringStackElement.getType() == StringStackElement.StringStackElementType.FIELD) {
            String simpleFieldName =
                stringStackElement.getValue().replace(ctClass.getName() + ".", "");
            ...
            if (propertyValue != null) {
              ...
              throw new DataFlowException(
                  "Can not resolve field value for " + stringStackElement.getValue());
            }
          }
        }
