...
public class RestEndpointService {
  ...
  private void resolveReturnTypeForMethodDeclaration(
      CompilationUnit cu, MethodDeclaration md, RestEndpoint restEndpoint) {
  ...
  // TODO: find FQ name?
  private void resolveArgumentsForMethodDeclaration(
      CompilationUnit cu, MethodDeclaration md, RestEndpoint restEndpoint) {
  ...
  private boolean hasRestControllerImport(CompilationUnit cu) {
    for (ImportDeclaration id : cu.findAll(ImportDeclaration.class)) {
      if (id.getNameAsString().equals("org.springframework.web.bind.annotation.RestController")) {
        return true;
      } else if (id.getNameAsString().equals("org.springframework.web.bind.annotation")
          && id.isAsterisk()) {
        return true;
      }
    }
  ...
  private String pathFromAnnotation(AnnotationExpr ae) {
    ...
    if (ae.isSingleMemberAnnotationExpr()) {
      return Helper.removeEnclosedQuotations(
