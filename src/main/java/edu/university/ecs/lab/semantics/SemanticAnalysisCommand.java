...
/**
 * SemanticAnalysisCommand serves as the entrypoint for the program see
...
public class SemanticAnalysisCommand implements QuarkusApplication {
  ...
  /**
   * This method serves as the main point of control for this application. It calls several
   * functions that separate each step of the Prophet Code Analysis Tool.
   *
   * <p>If no arguments are specific for the repoOutputDir and resultOutputDir then the current
   * executable's directory is used and repos and results will be output in default directories
   *
   * @param args commandline args, can be in one of the following formats:
   *     <ul>
   *       <li>[repo1, repo2, repo3, ...]
   *       <li>[repoOutputDir] [repo1, repo2, repo3, ...] [resultOutputDir]
   *     </ul>
   *
  ...
  /**
   * This method sets the paths from the arguments passed into this application, parses them
   * dependent on input format, and verifies they exist or creates them if they don't
   *
   * @param args commandline args, can be in one of the following formats:
   *     <ul>
   *       <li><repo1, repo2, repo3, ...>
  ...
  private void initAndValidatePaths(String... args) throws Exception {
    if (args.length == 1) {
      ...
      throw new IllegalArgumentException(
          "Usage: <repo1, repo2, repo3, ...> OR <repoOutputDir> <repo1, repo2, repo3, ...>"
              + " <resultOutputDir>");
    }
  ...
  /**
   * This method does all the data preprocessing from the cloned repositories and stores it into
   * MsCache. First it parses all the files that were cloned, then it builds flows off of that data,
  ...
  /**
   * main method entry, errors occuring within SemanticAnalysisCommand run() will be caught by
   * (exitCode, exception) -> {} and output here
   *
   * @param args command line arguments passed to run() method
   */
  public static void main(String[] args) {
    Quarkus.run(
        SemanticAnalysisCommand.class,
        (exitCode, exception) -> {
