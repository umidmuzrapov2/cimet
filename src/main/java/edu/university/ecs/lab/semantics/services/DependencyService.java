...
public class DependencyService {
    public static List<Pair<RestCall, Method>> getRawDependencies() {
        ...
        dependencies = CachingService.getCache().getRestCallList().stream().filter(restCall -> restCall.getApiEndpoint() != null).map(restCall -> {

            Optional<Method> m = CachingService.getCache().getMethodList().stream().filter(method -> method.getApiEndpoint() != null).filter(method -> method.getApiEndpoint().equals(restCall.getApiEndpoint())).findFirst();
    ...
    public static LinkedHashSet<String> getDependenciesList(List<Pair<RestCall, Method>> rawDependencies) {
    ...
    public static Map<String, Set<String>> getDependenciesMap(List<Pair<RestCall, Method>> rawDependencies) {
        Map<String, Set<String>> returnMap = new HashMap<>();

        for(Pair<RestCall, Method> p : rawDependencies) {
            if(p.b != null) {
                if (!returnMap.containsKey(p.a.getId().getProject())) {
                    returnMap.put(p.a.getId().getProject(), new LinkedHashSet<>(Set.of(p.b.getId().getProject())));
                } else {
                    returnMap.get(p.a.getId().getProject()).add(p.b.getId().getProject());
                }

                if (!returnMap.containsKey(p.b.getId().getProject())) {
                    returnMap.put(p.b.getId().getProject(), new LinkedHashSet<>(Set.of(p.a.getId().getProject())));
                } else {
