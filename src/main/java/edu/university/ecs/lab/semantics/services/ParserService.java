...
//        method.setIds();
    ...
    /**
     * From a node location, move up parser tree parentNode's
     * looking for MethodDeclaration if we are MethodCallExpr
     * and ClassOrInterfaceDeclaration/CompilationUnit for both
     *
    ...
    public static MethodLocation parseMethodLocation(Node node) throws IllegalArgumentException {
        ...
        if (classOrInterfaceDeclaration != null) {
            ...
            while (parentNode.isPresent()) {
                if (parentNode.get() instanceof CompilationUnit) {
                    CompilationUnit cu = (CompilationUnit) parentNode.get();
                    cu.getPackageDeclaration().ifPresent(pd -> methodLocation.setParentPackageName(pd.getNameAsString()));
