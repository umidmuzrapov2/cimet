...
/**
 * This class manages the building of the flows from controller down to repository using the entries
 * in MsCache
 *
 * <p>For method comments, (service/controller/repository) indicites either the MsClass type or
 * where the method call/definition would be located
 */
public class FlowBuilder {
  ...
  public void buildFlows() {
    ...
    for (MsFlowEntity msFlowEntity : msFlowEntities) {
      ...
      // msFlowEntity.getMsControllerMethod().getMethodName());
      // 2. get service method call in controller method
      Optional<MsMethodCall> serviceMethodCall =
          findServiceCall(msFlowEntity.getMsControllerMethod());
      if (serviceMethodCall.isPresent()) {
        ...
        if (serviceField.isPresent()) {
          ...
          // 4. get service class
          Optional<MsClass> msServiceClass =
              findService(msFlowEntity.getMsControllerServiceField());
          if (msServiceClass.isPresent()) {
            msFlowEntity.setMsService(msServiceClass.get());
            // 5. find service method name
            Optional<MsMethod> msServiceMethod =
                findMsServiceMethod(
                    msFlowEntity.getMsService(), msFlowEntity.getMsServiceMethodCall());
            if (msServiceMethod.isPresent()) {
              msFlowEntity.setMsServiceMethod(msServiceMethod.get());
              //                            System.err.println("SERVICE : " +
              // msServiceMethod.get().getMethodName());
              // 5. get rest calls
              ...
              Optional<MsMethodCall> repositoryMethodCall =
                  findMsRepositoryMethodCall(
                      msFlowEntity.getMsService(), msFlowEntity.getMsServiceMethod());
              if (repositoryMethodCall.isPresent()) {
                ...
                Optional<MsField> repositoryField =
                    findRepositoryField(
                        msFlowEntity.getMsService(), msFlowEntity.getMsRepositoryMethodCall());
                if (repositoryField.isPresent()) {
                  msFlowEntity.setMsServiceRepositoryField(repositoryField.get());
                  // 8. find repository class
                  Optional<MsClass> repositoryClass =
                      findRepositoryClass(msFlowEntity.getMsServiceRepositoryField());
                  if (repositoryClass.isPresent()) {
                    msFlowEntity.setMsRepository(repositoryClass.get());
                    // 9. find repository method
                    Optional<MsMethod> repositoryMethod =
                        findRepositoryMethod(
                            msFlowEntity.getMsRepository(),
                            msFlowEntity.getMsRepositoryMethodCall());
   ...
   */
  private Optional<MsMethod> findRepositoryMethod(
      MsClass msRepository, MsMethodCall msRepositoryMethodCall) {
    return MsCache.msMethodList.stream()
        .filter(
            n ->
                n.getMsId().getPath().equals(msRepository.getMsId().getPath())
                    && n.getMethodName().equals(msRepositoryMethodCall.getCalledMethodName()))
        .findFirst();
  }
   ...
   */
  private Optional<MsField> findRepositoryField(
      MsClass msService, MsMethodCall repositoryMethodCall) {
    return MsCache.msFieldList.stream()
        .filter(
            n ->
                n.getMsId().getPath().equals(msService.getMsId().getPath())
   ...
   */
  private Optional<MsMethodCall> findMsRepositoryMethodCall(
      MsClass msService, MsMethod msServiceMethod) {
    return MsCache.msMethodCallList.stream()
        .filter(
            n ->
                n.getMsId().getPath().equals(msService.getMsId().getPath())
                    && n.getParentMethodName().equals(msServiceMethod.getMethodName()))
        .findFirst();
  }
   ...
   */
  private Optional<MsMethod> findMsServiceMethod(
      MsClass msService, MsMethodCall controllerServiceMethodCall) {
    return MsCache.msMethodList.stream()
        .filter(
            n ->
                n.getMsId().getPath().equals(msService.getMsId().getPath())
  ...
  private List<MsRestCall> findRestCalls(MsMethod msMethodService) {
    return MsCache.msRestCallList.stream()
        .filter(
            n ->
                n.getParentClassName().equals(msMethodService.getClassName())
                    && n.getParentMethodName().equals(msMethodService.getMethodName()))
  ...
  private Optional<MsField> findServiceField(MsMethodCall msMethodCall) {
    return MsCache.msFieldList.stream()
        .filter(
            n ->
                n.getMsId().getPath().equals(msMethodCall.getMsId().getPath())
                    && n.getFieldVariable().equals(msMethodCall.getCalledServiceId()))
        .findFirst();
  }
        ...
        .filter(
            n ->
                n.getMsId()
                    .getPath()
                    .toLowerCase()
                    .contains(MsClassRoles.CONTROLLER.toString().toLowerCase()))
    ...
    return MsCache.msClassList.stream()
        .filter(
            n ->
                n.getMsId()
                    .getPath()
                    .equals(msFlowEntity.getMsControllerMethod().getMsId().getPath()))
        .findFirst()
        .get();
  }
  ...
  private Optional<MsMethodCall> findServiceCall(MsMethod msControllerMethod) {
    return MsCache.msMethodCallList.stream()
        .filter(
            n ->
                n.getParentMethodName().equals(msControllerMethod.getMethodName())
                    && n.getMsId().getPath().equals(msControllerMethod.getMsId().getPath()))
        .findFirst();
  }
}
