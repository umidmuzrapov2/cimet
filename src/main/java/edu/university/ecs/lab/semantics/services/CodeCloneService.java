...
//        msCodeClone.setCtrReturnTypeLiteralSimilarity(this.entitySimilarityChecker.calculateSimilarity(msCodeClone.getFlowA().getPackageName(), msCodeClone.getFlowB().getPackageName(), aCtrl.getReturnType(), bCtrl.getReturnType())); //
        ...
        int maxNumberAnnotations = Math.max(aCtrl.getAnnotations().size(), bCtrl.getAnnotations().size());
...
//        double argsSimilarity = this.entitySimilarityChecker.calculateArgumentsSimilarity(aCtrl.getParameterList(), bCtrl.getParameterList());
...
//
//                 if (aArguments.get(i).getReturnType() != null && bArguments.get(j).getReturnType() != null && !usedArguments.contains(j)
...
//        private double srvReturnTypeSemanticSimilarity;
        ...
        double same = 0.0;
        if (a.getReturnType() != null && b.getReturnType() != null && a.getReturnType().equalsIgnoreCase(b.getReturnType())) {
            same += 0.5;

            //Rico commented out
//            double similarityValue = this.entitySimilarityChecker.calculateSimilarity(msCodeClone.getFlowA().getPackageName(), msCodeClone.getFlowB().getPackageName(), a.getReturnType(), b.getReturnType());
...
//    	System.err.println(a.getHttpMethod() + " --- " + b.getHttpMethod());
        ...
        if (a.getApiEndpoint() != null && b.getApiEndpoint() != null && a.getApiEndpoint().replaceAll("\\s", "").toLowerCase().equals(b.getApiEndpoint().replaceAll("\\s", "").toLowerCase())) {
            similarity += 1.0;
