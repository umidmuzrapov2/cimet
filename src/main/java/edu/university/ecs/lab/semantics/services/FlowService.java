...
public class FlowService {
  ...
  public void buildFlows() {
    ...
    for (Flow flow : FlowEntities) {
      ...
      Optional<MethodCall> serviceMethodCall = findServiceCall(flow.getControllerMethod());
      if (serviceMethodCall.isPresent()) {
        ...
        if (serviceField.isPresent()) {
          ...
          // 4. get service class
          Optional<JClass> ServiceClass = findService(flow.getControllerServiceField());
          if (ServiceClass.isPresent()) {
            flow.setService(ServiceClass.get());
            // 5. find service method name
            Optional<Method> ServiceMethod =
                findServiceMethod(flow.getService(), flow.getServiceMethodCall());
            if (ServiceMethod.isPresent()) {
              ...
              Optional<MethodCall> repositoryMethodCall =
                  findRepositoryMethodCall(flow.getService(), flow.getServiceMethod());
              if (repositoryMethodCall.isPresent()) {
                ...
                Optional<Field> repositoryField =
                    findRepositoryField(flow.getService(), flow.getRepositoryMethodCall());
                if (repositoryField.isPresent()) {
                  ...
                  if (repositoryClass.isPresent()) {
                    ...
                    Optional<Method> repositoryMethod =
                        findRepositoryMethod(flow.getRepository(), flow.getRepositoryMethodCall());
   ...
   */
  private Optional<Method> findRepositoryMethod(
      JClass repository, MethodCall repositoryMethodCall) {
    return CachingService.getCache().getMethodList().stream()
        .filter(
            n ->
                n.getId().getLocation().equals(repository.getId().getLocation())
   ...
   */
  private Optional<Field> findRepositoryField(JClass Service, MethodCall repositoryMethodCall) {
   ...
   */
  private Optional<MethodCall> findRepositoryMethodCall(JClass Service, Method ServiceMethod) {
        ...
        .filter(
            n ->
                n.getId().getLocation().equals(Service.getId().getLocation())
                    && n.getMethodLocation()
                        .getParentMethodName()
                        .equals(ServiceMethod.getMethodName()))
        .findFirst();
  }
  ...
  private List<RestCall> findRestCalls(Method MethodService) {
    return CachingService.getCache().getRestCallList().stream()
        .filter(
            n ->
                n.getMethodLocation().getParentClassName().equals(MethodService.getClassName())
                    && n.getMethodLocation()
                        .getParentMethodName()
                        .equals(MethodService.getMethodName()))
  ...
  public JClass findController(Flow Flow) {
    return CachingService.getCache().getClassList().stream()
        .filter(
            n -> n.getId().getLocation().equals(Flow.getControllerMethod().getId().getLocation()))
        .findFirst()
        .get();
  }
  ...
  private Optional<MethodCall> findServiceCall(Method ControllerMethod) {
    return CachingService.getCache().getMethodCallList().stream()
        .filter(
            n ->
                n.getMethodLocation().getParentMethodName().equals(ControllerMethod.getMethodName())
