...
public class FlowService {
    ...
    private Optional<Method> findRepositoryMethod(JClass repository, MethodCall repositoryMethodCall) {
        return CachingService.getCache().getMethodList().stream()
                .filter(
                        n -> n.getId().getLocation().equals(repository.getId().getLocation())
    ...
    private Optional<Field> findRepositoryField(JClass Service, MethodCall repositoryMethodCall) {
        return CachingService.getCache().getFieldList().stream()
            .filter(n -> n.getId().getLocation().equals(Service.getId().getLocation())
    ...
    private Optional<MethodCall> findRepositoryMethodCall(JClass Service, Method ServiceMethod) {
        return CachingService.getCache().getMethodCallList().stream()
            .filter(n -> n.getId().getLocation().equals(Service.getId().getLocation())
                && n.getMethodLocation().getParentMethodName().equals(ServiceMethod.getMethodName()))
            .findFirst();
    }
     ...
     */
    private Optional<Method> findServiceMethod(JClass Service, MethodCall controllerServiceMethodCall) {
        return CachingService.getCache().getMethodList().stream()
            .filter(n -> n.getId().getLocation().equals(Service.getId().getLocation())
    ...
    private List<RestCall> findRestCalls(Method MethodService) {
        return CachingService.getCache().getRestCallList().stream()
            .filter(n -> n.getMethodLocation().getParentClassName().equals(MethodService.getClassName())
                && n.getMethodLocation().getParentMethodName().equals(MethodService.getMethodName()))
    ...
    private Optional<Field> findServiceField(MethodCall MethodCall) {
        return CachingService.getCache().getFieldList().stream()
            .filter(n -> n.getId().getLocation().equals(MethodCall.getId().getLocation())
    ...
    public List<Flow> findControllerMethods() {
        return CachingService.getCache().getMethodList().stream()
            .filter(n -> n.getId().getLocation().toLowerCase()
                .contains(ClassRole.CONTROLLER.toString().toLowerCase())).map(Flow::new).collect(Collectors.toList());
    }
    ...
    public JClass findController(Flow Flow) {
        return CachingService.getCache().getClassList().stream()
            .filter(n -> n.getId().getLocation().equals(Flow.getControllerMethod().getId().getLocation())).findFirst().get();
    }
    ...
    private Optional<MethodCall> findServiceCall(Method ControllerMethod) {
        return CachingService.getCache().getMethodCallList().stream()
            .filter(n -> n.getMethodLocation().getParentMethodName().equals(ControllerMethod.getMethodName())
