...
/**
 * Parse method information from a MethodDeclaration and generate msMethod object that will be saved
 * to cache
 */
public class MsMethodBuilder {

  /**
   * Parses method information and creates a MsMethod object representing the method
   *
    ...
    if (parentNode.isPresent()) {
      ...
      if (parentNode.isPresent()) {
        ...
        if (parentNode.get() instanceof CompilationUnit) {
          ...
          pd.ifPresent(
              packageDeclaration -> msMethod.setPackageName(packageDeclaration.getNameAsString()));
        }
